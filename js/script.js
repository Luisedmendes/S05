...
function reservarArmario() {
  ...
  const time = new Date();
  let prazo = new Date(time.setDate(time.getDate() + 1));
  armarioSorteado.time = time.toLocaleTimeString();
  armarioSorteado.deliveryTime = time.toLocaleTimeString();
  armarioSorteado.deliveryDate = time.toLocaleDateString();
  ...
  document.getElementById("resultado").innerText = `Olá, ${usuario.nome}!
   O armário ${armarioSorteado.id} foi reservado com sucesso ás ${armarioSorteado.time} horas
   O prazo de entrega será até ${armarioSorteado.deliveryDate} ás ${armarioSorteado.deliveryTime} horas
   `;
